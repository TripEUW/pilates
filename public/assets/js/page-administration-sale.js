            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        return `
                        <span class="">
                        <a href="#" onclick='setClient(` + JSON.stringify(data) + `)' class="btn btn-brand btn-elevate btn-icon-sm p-1"><i class="la la-plus"></i>Elegir</a>
                        </span>
                        `;
                    },
                }
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        <a href="#" onclick='addProduct(` + JSON.stringify(data) + `)' class="btn btn-brand btn-elevate btn-icon-sm p-1 m-2 bd-highlight"><i class="la la-plus"></i><br>Añadir</a>
                        <a href="#" onclick='selectedProduct(` + JSON.stringify(data) + `)' class="btn btn-brand btn-elevate btn-icon-sm p-1 m-2 bd-highlight"><i class="flaticon2-checkmark"></i></i><br>Elegir</a>
                        </span>
                        `;
                    },
                }
    ...
    products.forEach(itemProduct => {
        if (itemProduct.product.id == product.id) {
            ...
            if (unds == false) {
                ...
                itemProduct.cant = parseFloat(itemProduct.cant) + parseFloat(unds);
            }
            itemProduct.discount = discount;
            itemProduct.amount = calculeAmount(itemProduct.product.price, itemProduct.cant, itemProduct.discount, itemProduct.product.tax[0].tax);
            selectedProduct = itemProduct;
        }
    });

    if (!flagExist) {
        selectedProduct = { product: product, cant: ((unds == false) ? 1 : unds), amount: calculeAmount(product.price, ((unds == false) ? 1 : unds), discount, product.tax[0].tax), discount: discount, };
        products.push(selectedProduct);
        checkEmptyTable();
        tbodyAppend.append(`
<tr class="tr-table-add-product" id="tr_product_add_` + product.id + `">
<th scope="row" class="text-center">    
<button class="quit-element" onclick="quitProduct(` + product.id + `)"><i class="flaticon-cancel"></i></button>
</th>
<td class="text-center">` + product.id + `</td>
<td>` + product.name + `</td>
<td class="text-center">` + formatMoney(product.price) + `</td>
<td style="padding:0px !important;"><input type="number" min="1" oninput="changeCantProduct(` + product.id + `)" class="input-for-table text-center" step="1" id="cant_product_` + product.id + `" value="` + selectedProduct.cant + `"></td>
<td style="padding:0px !important;" id="cell_discount_` + product.id + `"><input type="number" min="1" oninput="changeDiscountProduct(` + product.id + `)" class="input-for-table text-center" step="1" id="discount_product_` + product.id + `" value="` + selectedProduct.discount + `"></td>
<td class="text-center">` + formatPercent(selectedProduct.product.tax[0].tax) + `</td>
<td class="text-center" id="amount_product_` + product.id + `">` + formatMoney(selectedProduct.amount) + `</td>
</tr>
`);
        ...
        var inputCantElement = document.getElementById("cant_product_" + selectedProduct.product.id);
        inputCantElement.value = selectedProduct.cant;

        var inputDisElement = document.getElementById("discount_product_" + selectedProduct.product.id);
        inputDisElement.value = selectedProduct.discount;

        var inputAmountElement = document.getElementById("amount_product_" + selectedProduct.product.id);
...
function selectedProduct(product) {
    ...
    showToast(1, 'Ahora clic en añadir para agregar el producto a la tabla, cambie el % de descuento o unidades que requiere el cliente antes de añadir (puede cambiar los valores de cántidad o descuento después de añadir)',10000);
}
...
function addSelectedProduct() {
    if (selectedProductForAdd == false) {
        ...
        addProduct(selectedProductForAdd, ($('#discount').val()) ? $('#discount').val() : 0, ($('#unds').val()) ? $('#unds').val() : 1);
        $('#name_p').val("");
        $('#unds').val(0);
        $('#discount').val(0);
        $('#price').val("");
...
function changeCantProduct(idProduct) {
    ...
    products.forEach(itemProduct => {
        if (itemProduct.product.id == idProduct) {
            var inputCantElement = document.getElementById("cant_product_" + idProduct);
            var inputAmountElement = document.getElementById("amount_product_" + idProduct);
            itemProduct.cant = (inputCantElement.value) ? inputCantElement.value : 0;
            itemProduct.amount = calculeAmount(itemProduct.product.price, itemProduct.cant, itemProduct.discount, itemProduct.product.tax[0].tax);
...
function changeDiscountProduct(idProduct) {
    ...
    products.forEach(itemProduct => {
        if (itemProduct.product.id == idProduct) {
            var inputDiscElement = document.getElementById("discount_product_" + idProduct);
            var inputAmountElement = document.getElementById("amount_product_" + idProduct);
            itemProduct.discount = (inputDiscElement.value) ? inputDiscElement.value : 0;
            itemProduct.amount = calculeAmount(itemProduct.product.price, itemProduct.cant, itemProduct.discount, itemProduct.product.tax[0].tax);
            inputAmountElement.innerHTML = formatMoney(itemProduct.amount);
            //console.log("change cant: "+itemProduct.cant);
        }
...
function issueInvoice() {
    showOverlay();
    var amountSend = 0;
    products.forEach(product => {amountSend += parseFloat(product.amount);});
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (res.error != false) {
                ...
                resetSale();
                $("#download-print-invoice-btn").attr("href", res.path_download); // Set herf value
